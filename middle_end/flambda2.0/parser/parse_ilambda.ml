...
let of_raw_location loc =
  Debuginfo.Scoped_location.of_location ~scopes:[] loc
  ...
  try
    ...
    let program =
      ...
      | Parser.Error ->
        ...
        Error (Parsing_error (of_raw_location loc))
      | Lex.Error (error, loc) ->
        Error (Lexing_error (error, of_raw_location (make_loc loc)))
    in
    close_in ic;
    program
  with
